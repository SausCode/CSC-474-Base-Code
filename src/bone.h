...
public:
	...
	unsigned int index;
	mat4 *mat = NULL;
	float percentToInt = .9;

	quat inter(quat prev, quat curr, quat next)
	{
		return exp((log(next * inverse(curr)) + log(prev * inverse(curr))) / -4.0f) * curr;
		// I have no idea what this does, I found the formula on https://www.3dgep.com/understanding-quaternions/
	}
	...
	void play_animation(int keyframenumber, int current_animation, int next_animation) {
		...
		if ((float)keyframenumber / (float)(next_frames - 1) >= percentToInt) {
			float t = (float)(keyframenumber - (int)((next_frames - 1) * percentToInt)) / (float)((next_frames - 1)- (int)((next_frames - 1) * percentToInt));
	...
	void play_slowmo_animation(int keyframenumber, int current_animation, int next_animation) {
		...
		if ((float)keyframenumber / (float)((next_frames - 1) * 10) >= percentToInt) {
			// We're in the last 10th of our animation - let's interpolate the rest to the beginning
			float t = (float)(keyframenumber - (int)((next_frames - 1) * percentToInt)) / (float)((next_frames - 1) * 10.f - (int)((next_frames - 1) * percentToInt));
